/* Shandalar game layer: the per-owner slot table.
 *
 * Built /Od like the rest of the game layer -- see docs/decompilation.md.
 *
 * ONE HEADER PER MODULE, DELIBERATELY -- see the note in sidlib/sidcore.h.
 *
 * This file used to say the reason "applies with less force here (unoptimised
 * code does not re-roll its register allocation when a declaration appears)".
 * THAT WAS WRONG, measured 2026-07-28. Adding the card-script typedef, two
 * externs and one prototype to this header took Game_ResetState from 95.15% to
 * 100% with reset.c itself untouched -- it flipped the operand order MSVC chose
 * for an array index, which was that function's entire residual. Reverting only
 * the new struct member kept the 100%, so it was the plain declarations that
 * did it, exactly as at /O2.
 *
 * So /Od is perturbed by header contents too, and the discipline matters here
 * as much as it does in sidlib. It cuts both ways: this time an unrelated
 * addition helped, and next time it will hurt. Measure the whole target after
 * touching this file, not just the function being worked on.
 */
