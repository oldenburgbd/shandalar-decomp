/* Shandalar game layer: card behaviour scripts, cards43.
 *
...
/* Tests two bits of the card's flags together -- (flags & 0x42) != 0x42, so it
 * fires unless BOTH are set -- then looks the card's mask up in the table at
 * 0x008e44b8 and requires that entry not be 0x70. */
...
/* --- six more, all guard-then-act --------------------------------------- */

/* Reads a byte from the card behind the current slot's field6c. The chain is
 * four terms and every failure lands on the same `return 0`. */
...
/* The helper's return value indexes g_game77ed80, and the row is `1 - a` --
 * the other side of a two-sided table, where 0x0047903d below uses `a`. */
...
/* The only one of the eight that returns anything but 0: kind 0x74 answers 1.
 * A script's return value is therefore meaningful for at least some kinds,
 * which the four call sites read so far did not show. */
...
/* A pair that differ only in the constant handed to the first helper -- 3 here,
 * 5 in 0x0044e945 -- and then run the identical three-call tail. */
...
/* The four-argument helper takes the OTHER side's table entry: the row index is
 * `1 - a` both for the lookup and for the argument itself. */
