/* Shandalar game layer: card behaviour scripts, cards49.
 *
...
/* No `kind` test at all -- this one fires on every event aimed at its slot.
 * Note the two comparisons are written round opposite ways, the global first
 * and then the parameter first; that is the original's asymmetry, not ours. */
...
/* --- guard-then-act ------------------------------------------------------
 *
 * The common shape: check `kind`, check that the slot the script was invoked
 * on is the one the globals point at, then do one thing. Where a script has
 * already established that a parameter equals a global it indexes with the
 * PARAMETER in some places and the GLOBAL in others; both are reproduced as
 * written, because although they select the same slot they compile
 * differently.
 *
 * THE COMPARISON OPERAND ORDER IN THESE IS NOT REACHABLE FROM THE SOURCE, and
 * it is most of what the three sub-100% scripts below are missing. Whether the
 * global or the parameter ends up in the register, and which is the memory
 * operand of the `cmp`, is chosen by the compiler: writing `g == p` and
 * `p == g` produce the same instructions, and the choice is not even
 * consistent between two functions whose first three terms are identical
 * (0x004974ab matches on its second term and 0x0049c127 does not, from the
 * same source text). Flipping all three mismatching terms was measured on
 * 2026-07-28 and moved none of them -- 60.61%, 76.60% and 79.41% before and
 * after, to the digit. This is the case CLAUDE.md's breadth-first rule is
 * about; do not spend another build on it.
 *
 * NEITHER IS THE ORDER THE TWO INDEX SUBEXPRESSIONS ARE EVALUATED IN. Writing
 * g_gameSlots[a][b], MSVC computes either the column into eax and then the row
 * into ecx, or the reverse, and which one it picks moves for the whole
 * translation unit when this file's header changes. Adding six scripts and
 * four declarations flipped every script from column-first to row-first: it
 * took 0x0049c127 from 79.41% to 100% and 0x0048c465 from 100% to 72.97% in
 * the same build. The originals are not consistent either -- 0x0049744a is
 * row-first and 0x0048c465 column-first, from the same C construct -- so no
 * single setting matches them all and chasing it is not worth a build.
 *
 * The lesson is about the METRIC, not the compiler: that build lost two
 * byte-exact functions and still added 843 matched bytes to Shandalar.exe.
 * Judge a change here on matched bytes, never on the per-function table. */

/* Clears bit 5 of a counter global when invoked on the current slot, which
 * this one identifies three ways over: column, row, and a third global that
 * also has to equal the row. */
...
/* Three separate `return 0` statements, which is what the three distinct
 * xor/jmp pairs in the original require -- collapsing them to one changes the
 * shape. */
...
/* Sets a bit whose position the helper chooses, calls a second helper, then
 * puts the counter back exactly as it was -- so the bit exists only for the
 * duration of that inner call. The save/restore is what the local is for.
 *
 * 0x0049c630 below is the same function with 1 replaced by 5 throughout. */
...
/* No `kind` test on the first block at all -- it runs on every event, guarded
 * only by what the two helpers answer. The second block asks the same question
 * of the other row. */
