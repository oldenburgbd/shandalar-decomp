...
/* Two independent blocks, not an else-if: a slot event on 0x22 or 0xc7, and a
 * counter bit on 0x34. Both re-test the same two globals rather than sharing
 * the check. */
...
/* The one place so far a script uses ||, and it is worth reading twice: the
 * counter is bumped when the slot's bit 4 is CLEAR, or, if it is set, when the
 * card behind it has bit 1 set. Short-circuit order is visible in the
 * codegen -- the first test jumps forward INTO the body, the second jumps past
 * it. */
