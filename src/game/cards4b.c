/* Shandalar game layer: card behaviour scripts in the 0x4Bxxxx range.
 *
 * Split by address range into small translation units on purpose: MSVC 4.2
 * re-rolls codegen across a whole TU when anything changes, so keeping the
 * units small keeps one new script from perturbing the others. The original
 * evidently built one source file per card (identical small handlers appear
 * at several distinct addresses instead of being shared).
 *
...
/* Verified against reports/ghidra/decomp/004b29c6.c: the -1, -1 tail on the
...
/* The 0x74 arm returns 1 by two routes; the original has a dead jmp between
 * them, so the source used an else branch, not a fallthrough. */
...
/* Near-pair with 0x004b3902 below: same shape, different globals, constants,
 * callback, and counters. Verified against reports/ghidra/decomp/004b3a96.c. */
...
/* Verified against reports/ghidra/decomp/004b01c2.c. ig is passed to field78
 * uninitialized here, mirroring Ghidra's local_c; the raw 0x8aa920 literal is
 * intentional. */
