/* Card behaviour scripts, cards48 address range.
 *
 * Kept as a small TU on purpose: MSVC 4.2 /Od codegen is sensitive to the
 * whole translation unit, so isolating scripts by address range keeps one
 * edit from perturbing already-matched functions elsewhere.
 *
 * CUR/SRC macros come from game/gameslot.h. */

#include "game/gameslot.h"

/* Bumps g_game7a01e0 for kinds 0x32/0x33 when the current slot matches. */
...
/* One guard, then three kind cases in a row (the last two identical). */
...
/* Do-nothing handlers. 0x00488ca0 serves as the table's null entry; the
 * others are byte-identical copies, consistent with one source file per
 * card in the original. */
...
/* Kind 0x6e: if the current slot still matches the arguments, clear its
 * field38 when the source slot's card has flags bit 1 set. The field6c/field3c
 * fallback pair is written out inline, as elsewhere. */
...
/* Like 0x0048b610 but on the other global pair, passing b and testing > 1. */
...
/* Note for 0x004b47b1: it hands 0x004b484b (a real four-plus-argument
 * function) to Game_Helper488b70, so some entries reached via the helpers
 * are callbacks, not card-table scripts. */

/* Paired with 0x0048b673: same query constant 0x32, but this one asks about
 * the current row/column and tests > 2. */
...
/* Mirror of 0x0048b610: same guards plus a non-zero card byte, then tests
 * bit 0x20 of the helper's result. */
...
/* --- cardgen output (ghidra-work/scripts/cardgen.py) ---------------------
 * Machine-reconstructed from the disassembly; reccmp is the check. Names are
 * addresses since nothing establishes meaning. */
